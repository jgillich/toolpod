...
| `repos` | map | Extra apt sources to enable before installing `packages:` (e.g. `repos: { mise: { extrepo: mise } }`). v1 supports `extrepo:` catalog names, resolved at build time from the extrepo catalog for the base image's Debian version; inline `url`/`key_url` repos are schema-ready but rejected at build time. |
...
The `extrepo` package is not needed in the base image — tpod resolves the
catalog itself at build time. Inline custom repos (`url`/`key_url`/`suites`/`components`)
are schema-ready but rejected at build time until v2. The `mise` profile itself
uses this — the base image no longer carries `mise`; each launch builds/uses a
derived image that installs it.
