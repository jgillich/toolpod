...
- `docs/` — design notes.
- `Dockerfile` — the mise base image (bare debian:13 + extrepo + ca-certificates; `mise.toml` pins `opencode` for development). `mise` itself installs via the `mise` profile's `repos:`/`packages:` into a derived image.
